...
func TestRateLimiter(t *testing.T) {
	...
	backend1 := httptest.NewServer(
		httpx.NewRouter().Route("/", httpx.Text("success"), m.Recover, m.TraceIDSetter, m.RealIP, m.SetHeader("Server", "backend", false)),
	)
	defer backend1.Close()

	// PROXY
	r := httpx.NewRouter()
	r.Use(m.Recover, m.TraceIDSetter, m.SetHeader("Server", "proxy", true), m.RateLimiter(
		time.Second,
		1,
		2*time.Second,
